...
if not isAlreadyLoaded then  
	...
	local util = (ENV.QKUtil) or (function()  
		local url = GITHUB .. "Utilities/refs/heads/main/Utility/Main.lua"  
		local rf, IF = ENV.readfile or ENV.read_file,  
			ENV.isfile or ENV.is_file  
  
		return loadstring(  
			rf and IF and IF("QUtil/Utility.lua") and rf("QUtil/Utility.lua")  
			or ENV.request and ENV.request({  
				Url = url,  
				Method = "GET"  
			}).Body  
			or game:HttpGet(url)  
	...
	local run_on_actor = ENV.run_on_actor  
		...
		topLabel.Changed:Connect(function()  
			...
			if #splitted == 3 then  
				...
				if difficultyStart then  
					...
					display.SongDifficulty =  
						l1:sub(difficultyStart + 9, difficultyEnd - 1)  
				end  
				...
				if colorStart then  
					...
					local color = l1  
						:sub(colorStart + 14, colorEnd - 2)  
						:gsub("\n\r\f\t\s\0 ", "")  
						:lower()  
  
					if color:sub(1, 3) == "rgb" then  
						display.SongNameColor =  
							c3r(unpack(color:sub(5, -2):split(",")))  
				...
				display.SongDuration =  
					max(display.TimeLeft, display["SongDuration"], 0)  
  
				display.SongRealDuration =  
					round(display.SongDuration * display.Rate)  
  
				settings.SongRealDuration =  
					display.SongRealDuration  
  
				timePassed =  
					round(  
						(display.SongDuration - display.TimeLeft)  
						* display.Rate  
					)  
...
if not renv then  
	return loadFail(  
		"getrenv is missing in your executor environment"  
	)  
end  
...
while wait(0.01) do  
	...
	local myField =  
		framework.VSRG.GameHandler.Field  
	...
	local function onInput(v)  
		...
		while not input and vsrgConnection do  
			for idx, val in v:GetChildren() do  
				if val.Name:match("Gamepad")  
					or tick() - start > 2  
					and val.Name:match("Keyboard")  
					or tick() - start > 4  
				then  
					input = val  
					break  
	...
	local function hookInputs(vsrgContext)  
		...
		vsrgConnection =  
			vsrgContext:GetPropertyChangedSignal("Parent")  
				:Connect(function()  
		...
		vsrgConn2 =  
			vsrgContext.ChildAdded:Connect(onInput)  
	end  
	...
	local function append(table, value, size)  
		local size =  
			round(max(tonumber(size) or 0, 1))  
	...
	local badNoteAssets = {  
		"rbxassetid://103483801062498",  
		"rbxassetid://88530467220950",  
		"rbxassetid://109130876544260",  
		"rbxassetid://120222801097284",  
		"rbxassetid://101951481332606"  
	}  
	...
	local function isDownS(data)  
		...
		for laneIndex, note in data.Notes do  
			if note.Note.Position.Y.Scale  
				< receptor.Position.Y.Scale + 0.5  
			then  
				topOnes += 1  
			end  
		end  
	...
	local function sortLane(data)  
		local function sortF(a, b)  
			local cond =  
				a.Note.Position.Y.Scale  
				< b.Note.Position.Y.Scale  
	...
	local function flushRenderQueue(  
		queue,  
		isMine  
	)  
		local maxRendersAllowed =  
			calcRender(isMine)  
  
		local canRender =  
			maxRendersAllowed  
			- alreadyRendered[isMine]  
		...
		dlanes[boolIdx[isMine]]  
			.NotesVisible =  
			alreadyRendered[isMine]  
  
		display.NotesVisible =  
			dlanes[boolIdx[true]].NotesVisible  
			+ dlanes[boolIdx[false]].NotesVisible  
	end  
  
	noteAdded:Connect(function(note)  
		local isMine = note.IsMine  
		local queue = renderQueue[isMine]  
		local noteObj = note.Note  
  
		noteObj.Visible = notesperf <= 0  
  
		insert(queue, noteObj)  
	...
	-- Forward declaration: onNote can run before the detector is assigned.
	local getLongNoteBody
	local getHoldTime

	local function onNote(  
		note,  
		data,  
		sharedData,  
		isNew,  
		...
		local sprite =  
			note:WaitForChild("LayeredSprite")  
				:WaitForChild("1")  
  
		local isBlack =  
			sprite.ImageColor3 == black  
  
		local n =  
			dataIndex[isMine] .. "Notes"  
  
		local n2 =  
			dataIndex[isMine] .. "BlackNotes"  
		...
		if isNew then  
			local isDownScrollSpawn =  
				note.Position.Y.Scale  
				< data.Receptor.Position.Y.Scale + 0.5  
  
			if isDownScrollSpawn  
				~= data.IsDownScroll  
			then  
				data.IsDownScroll =  
					isDownScrollSpawn  
		...
		-- ADDED:  
		-- Keep checking this note while it exists.  
		-- Some long-note bodies can appear/update after  
		-- the initial note object is created.  
		spawn(function()  
			for i = 1, 30 do  
	...
	local function onLane(  
		...
		data.LanesCount =  
			max(data.LanesCount, laneNum)  
  
		local Notes = { }  
  
		local receptor =  
			lane:WaitForChild("Receptor")  
		...
		local notes =  
			lane:WaitForChild("Notes")  
  
		if not sharedData.Working then  
			return  
		end  
	...
	spawn(function()  
		while true do  
			...
			ho =  
				actions  
				and actions[3] / 1000  
				or settings.HitOffset  
  
			spr = settings.Spray  
			notesperf =  
				settings.Performance.Notes  
			psick =  
				settings.PerfectSick  
			hdv =  
				settings.HoldDuration.Value  
			hdr =  
				settings.HoldDuration.Random  
			ap =  
				settings.AutoPlay  
	...
	canHit = function(note, data, far)  
		local x =  
			UDimToVector2(  
				data.Receptor.Position  
			)  
			+ v2(  
				useX and 0.5 or 0,  
				0.5,  
	...
	-- =========================================  
	-- OPEN-SOURCE LONG NOTE DETECTION
	-- =========================================
	...
	getLongNoteBody = function(Note)
		...
		if cached
			and cached.Parent == Note
			and cached:IsA("GuiObject")
		then
			return cached
		end

		-- Exact direct-child size test used by the supplied
		-- open-source Funky Friday hitNote implementation.
		-- There is intentionally no minimum pixel-size cutoff,
		-- so very short holds are not discarded.
		for _, child in Note:GetChildren() do
			if child:IsA("GuiObject")
				and child.Parent == Note
			then
				if child.Size ~= UDim2.fromScale(1, 1) then
					holdBodyCache[Note] = child
					return child
				end
			end
		end

		holdBodyCache[Note] = nil
		return nil
	end

	getHoldTime = function(Note, data)
		if not Note
			or not data
		then
			return 0
		...
		-- Exact duration formula from the supplied open-source
		-- Funky Friday autoplay implementation.
		return math.abs(
			body.Size.Y.Scale
				/ denominator
		) + 0.35
	...
	-- Prevent duplicate scheduling when normal processing and lag recovery overlap.
	local lagRecoveryPending = setmetatable({}, { __mode = "k" })
	...
	local function hitNote(  
		...
		-- ADDED:  
		-- Do the final long-note scan immediately before  
		-- committing the hit. This is the important late  
		-- detection path.  
		local holdTime =  
			getHoldTime(  
				Note,  
		...
		-- Improved long-note detection:  
		-- detected independently for this exact note.  
		time += holdTime  
	...
	-- =========================================  
	-- ADDED: LAG / SPAM NOTE RECOVERY  
	-- =========================================  
  
	local lagRecoveryStamp =  
		setmetatable({}, {  
			__mode = "k"  
		})  
  
	local function recoverLaggedNotes(lane)  
		-- Don't run recovery for a normal frame.  
		-- This keeps normal hit timing untouched.  
		if renderDelta <= 0.08 then  
			return  
		end  
		...
		-- The slower the frame, the further we allow  
		-- recovery to reach. It is capped so a huge freeze  
		-- does not suddenly hit ancient notes.  
		...
		for i = 1, #notes do  
			...
			if note  
				and note.Note  
				and note.Note.Parent  
				and not note.Hit  
			then  
				local x =  
					UDimToVector2(  
						lane.Receptor.Position  
					)  
					+ v2(  
						useX and 0.5 or 0,  
						0.5,  
				...
				if lane.ScrollSpeed > 0 then  
					...
					-- Only recover notes that have actually  
					-- crossed the receptor and are close enough  
					-- to have plausibly been skipped by a lag spike.  
					if behind  
						and dist <= recoveryLimit  
					then  
						local hit,  
							far,  
							canDist,  
		...
		-- Process every recoverable note rather than stopping  
		-- at the first note. This is what prevents a spam wall  
		-- from being blocked by one note that crossed during lag.  
		for _, entry in recovered do  
			local note =  
				entry.Note  
	...
	function playLane(lane)  
		-- ADDED:  
		-- Recover notes that crossed during a lag spike  
		-- before normal queue processing.  
		recoverLaggedNotes(lane)  
		...
		for i = 1, #notes do  
			...
			if hit then  
				...
				-- ADDED:  
				-- If this frame was slow, don't let the first  
				-- blocked note prevent later spam notes from  
				-- being examined by the recovery system.  
				if renderDelta > 0.08 then  
					continue  
				end  
	...
	local function processLane(  
		...
		local receptorTravel =  
			receptorEnd - receptorStart  
  
		if not modChart  
			and receptorTravel.Magnitude  
				> 0.001  
		then  
			spawn(mcdf)  
  
			if modChartDetectBuffer  
				> estFps * 10  
			then  
				modChart = true  
	...
	local function statsAdded(stats, cons)  
		if stats:FindFirstChild("Title") then  
			return  
		end  
		...
		cons[#cons + 1] =  
			re:Connect(function()  
				...
				if hs then  
					...
					rows:Rendered(  
						display.NotesRendered  
							== display.NotesVisible  
						and display.NotesRendered  
						or "Rendered: "  
							.. display.NotesVisible  
							.. " ("  
							.. display.NotesRendered  
	...
	local function onWindow(window)  
		...
		sharedData.StoppedWorking:Once(  
			function()  
				songStopped:Fire(  
					sharedData  
				)  
	...
	local function vf(ch)  
		if ch.ClassName == "InputContext"  
			or ch.Name == "VSRGContext"  
		then  
			hookInputs(ch)  
		elseif ch.Name == "Window"  
			and ch:WaitForChild("Game", 1)  
			and ch.Game:WaitForChild(  
				"Fields",  
				1  
			)  
		then  
...
if CoreGui:FindFirstChild(  
	...
	CoreGui.InfernumAutoplay:Destroy()  
end  
...
InfernumGui.ResetOnSpawn = false  
  
if gethui then  
	InfernumGui.Parent = gethui()  
elseif syn and syn.protect_gui then  
	syn.protect_gui(InfernumGui)  
	InfernumGui.Parent = CoreGui  
else  
	InfernumGui.Parent = CoreGui  
end  
